...
from visitors_entry_system.users.auth.tokens import get_token_for_password_reset
...
def test_user_password_reset_and_confirm(client, settings, mocker):
    url = reverse('auth-password-reset')
    user = f.create_user(email='test@example.com')
    mock_email = mocker.patch('visitors_entry_system.users.auth.services.send_mail')
